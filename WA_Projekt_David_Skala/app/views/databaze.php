...
$inzerat_model = new inzeraty($db);
$inzeraty = $inzerat_model->getAll();
?>
...
<body style="background-color: #FFDFDE;">
    ...
    <h2>Seznam uživatelských nabídek</h2>

    <a href="vytvorit_inzerat.php" style="display: inline-block; background-color: #6A7BA2; color: white; padding: 0.5rem 1rem; border-radius: 4px; text-decoration: none; cursor: pointer; margin-bottom: 10px;">
    Vložit nabídku
    </a>

    <span style="color: #000; margin-left: 10px; line-height: 2.5;">(Jen registrovaní uživatelé)</span>

        <?php if (!empty($inzeraty)): ?>
            <table class="table custom-table table-hover">
                ...
                <tbody>
                    <?php foreach ($inzeraty as $inzerat): ?>
                        <tr>
                            <td><?= htmlspecialchars($inzerat['id_cislo']) ?></td>
                            <td><strong><?= htmlspecialchars($inzerat['nazev_vozu']) ?></strong></td>
                            <td><?= htmlspecialchars($inzerat['rok_vyroby']) ?></td>
                            <td><?= htmlspecialchars(number_format($inzerat['najeto'], 0, '', ' ')) ?> km</td>
                            <td><strong><?= htmlspecialchars(number_format($inzerat['cena'], 0, '', ' ')) ?> Kč</strong></td>
                            <td><?= htmlspecialchars($inzerat['kontakt']) ?></td>
                        </tr>
                    <?php endforeach; ?>
                </tbody>
            </table>
        <?php else: ?>
            <div class="alert alert-info">Žádný inzerát nebyl nalezen.</div>
        <?php endif; ?>
    </div>
